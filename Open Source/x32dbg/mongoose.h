...
// 兼容 VS2013：用 __FUNCTION__ 替代 __func__
#if _MSC_VER <= 1800  // _MSC_VER=1800 对应 VS2013
#define __func__ __FUNCTION__
#endif

// 必须放在所有头文件包含之前！
#define WIN32_LEAN_AND_MEAN  // 禁用 Windows 冗余头文件（包括 winsock.h）
#define _WINSOCKAPI_       // 阻止 winsock.h 被其他头文件（如 windows.h）间接包含
#include <winsock2.h>       // 显式包含新版 Winsock 2.2 头文件
#include <ws2tcpip.h>      // 可选（若使用 TCP/IP 高级功能，如 inet_pton）
#pragma comment(lib, "ws2_32.lib")  // 显式链接 Winsock 2 库（避免链接错误）
